...
function _sortApplicants(list){
  ...
  if(applicantSort.field === 'date'){
    sorted.sort(function(a, b){
      var ka = toIsoDate(a.date);
      var kb = toIsoDate(b.date);
      if(ka === kb) return 0;
      var cmp = ka > kb ? 1 : -1;
...
function renderApplicants(list){
  ...
  sorted.forEach(function(a){
    ...
    var delBtn = '<button class="btn btn-danger" style="font-size:11px;padding:4px 10px" onclick="deleteApplicantRow(\'' + escAttr(a.eno) + '\', \'' + escAttr(a.at) + '\')">삭제</button>';
    ...
    tr.innerHTML =
      '<td class="app-check" style="text-align:center"><input type="checkbox" ' + checked + ' data-key="' + escAttr(key) + '" onchange="toggleApplicantSelection(this)" style="accent-color:var(--blue);cursor:pointer"></td>' +
      '<td style="font-family:monospace;font-size:12px">' + a.eno + '</td>' +
      '<td style="font-weight:600">' + a.name + '</td>' +
      '<td>' + toKoreanLabel(a.date) + '</td>' +
      '<td>' + a.room + '</td>' +
      '<td>' + famDisplay + '</td>' +
      '<td>' + infantBadge  + '</td>' +
      '<td>' + toddlerBadge + '</td>' +
...
function exportExcel(){
  ...
  var rows = APPLICANTS.map(function(a){
    return [a.at, a.eno, a.name, toKoreanLabel(a.date), a.room, a.fam, a.infant?'예':'아니오', a.toddler?'예':'아니오', a.status, a.rank];
  });
